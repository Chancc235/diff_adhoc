...
class DecisionTransformer(TrajectoryModel):
    ...
    def __init__(
        ...
        self.embed_goal = torch.nn.Linear(self.state_dim * num_agents, hidden_size)
    ...
    def forward(self, states, actions, goal, timesteps, attention_mask=None):
        ...
        # get predictions
        goal_preds = self.predict_goal(x[:,2])  # predict next goal given state and action
        state_preds = self.predict_state(x[:,2])    # predict next state given state and action
        action_preds = self.predict_action(x[:,1])  # predict next action given state

        return state_preds, action_preds, goal_preds
    ...
    def get_action(self, states, actions, goal, timesteps, **kwargs):
        ...
        _, action_preds, goal = self.forward(
            states, actions, goal, timesteps, attention_mask=attention_mask, **kwargs)

        return action_preds[0,-1]


class DecisionTransformer_lbf(TrajectoryModel):
